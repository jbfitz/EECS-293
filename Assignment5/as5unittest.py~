...
class InvalidTestCase(TestCase):
	...
	def test_cell_passage_invalid(self):
		...
		assert_equal(self.cell_two.status, Status.INVALID_TIME)
...
class TwoCellDeadEnd(TwoCellCase):
	...
	def test_dead_end_cell(self):
		...
		assert_equal(self.cell_one.status, Status.OK)
		assert_equal(self.cell_two.status, Status.ALREADY_VALID)
...
class FourCellOutsideMazeCase(FourCellTestCase):
	...
	def build_maze(self):
		...
		self.cells[1].add_passages({self.cells[2]: 5, self.cells[3]: 40})
		self.cells[2].add_passages({self.cells[3]: 5, self.cells[0]: 80})
		self.cells[3].add_passages({self.cells[0]: MAX_VALUE})

		self.maze.add_cells([self.cells[0], self.cells[1], self.cells[2]])
		self.first_route = self.maze.route_first(self.cells[0])


	def test_four_maze(self):
		assert_equals(self.first_route.valid, True)
		assert_equals(len(self.first_route.get_cells()), 4)
		assert_equals(self.maze.check_valid_exit(self.cells[3]), True)
...
class NetworkMazeCase(TestCase):
	...
	def connect_network_maze(self):
		...
		self.maze.add_cells(self.cells)
		with patch.object(self.maze, 'grab_random', self.random_patched_method):
	...
	def test_network_maze(self):
		...
		assert_not_equal(self.first_route.travel_time(), self.first_route.travel_time_random())

	def test_network_average_time_case(self):
		assert_equals(self.maze.check_valid_exit(self.exit), True)
		assert_equals(self.maze.average_exit_time(self.exit, self.maze.grab_greedy), 1)
